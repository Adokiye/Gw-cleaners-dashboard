...
class Login extends Component {
    constructor(props) {
        super(props);
        this.state = { apiResponse: "", email: '', password: '', error: '', error_div: false };
    }
    log(){
    //    console.log("djdjd")
        ...
        if (regg.test(this.state.email) === false) {
         this.setState({error: 'Incorrect Credentials', error_div: true})
            ...
            this.setState({error: 'Please fill all required fields', error_div: true})
        } else{
 ...
 //           this.props.navigation.navigate("Dashboard", {});
         ...
         this.setState({error: JSON.stringify(error.message), error_div: true})
   //             this.setState({ regLoader: false }); 
                if (error.response) {
                    this.setState({error: JSON.stringify(error.response.message), error_div: true})
